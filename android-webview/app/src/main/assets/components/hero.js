const HeroComponent = ({ props }) => ({
  next(message = {}) {
    if (message.type === "setTab") {
      const nextTab = message.value === "alugar" ? "alugar" : "comprar";
      props.goToRoute?.("home", { operation: nextTab });
    }

    const routeOperation = props.getRouteInfo?.().operation;
    const tab = routeOperation === "alugar" ? "alugar" : "comprar";
