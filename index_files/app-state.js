"use strict";
  ...
  const parseRoute = () => {
    ...
    const propertyId = route === "imovel" || route === "imovel-editar" ? (hashPropertyId || queryPropertyId) : queryPropertyId;
    return { route, propertyId, brokerId: params.get("brokerId") || null, operation: params.get("operation") || null };
  };
  ...
  const routeAttrs = (visible) => /*html*/`aria-hidden="${visible ? "false" : "true"}" style="display:${visible ? "block" : "none"};"`;
  ...
  const summarizeItem = (collection, item, index = 0) => {
    if (collection === "properties") {
      return {
        ...
        value: /*html*/`${item.price || "Sem preco"}${item.area ? ` · ${item.area}m2` : ""}`,
  ...
  const collectionFormTitle = (collection, mode) => /*html*/`${mode === "edit" ? "Editar" : "Novo"} ${DASHBOARD_COLLECTION_SCHEMAS[collection]?.itemLabel || "item"}`;
