    ...
    return {
      next(message = {}) {
        ...
        const pager = Array.from({ length: totalPages }, (_, index) => index + 1)
          .map((item) => /*html*/`<button class="${item === page ? "active" : ""}" type="button" data-cid="listing" data-message="setPage" data-value="${item}">${item}</button>`)
          .join("");
                ...
                <div class="listing-layout">
                  <aside class="filter-box" aria-label="Filtros">
                    ...
                    <div class="filter-stack">
                      <div class="mini-field"><label>Tipo</label><div class="check-list">${["Casa", "Apartamento", "Terreno", "Cobertura", "Sala comercial"].map((kind) => /*html*/`<label> ${kind}<input type="checkbox" data-cid="listing" data-message="filter" data-name="kind" value="${kind}" ${filters.kinds.has(kind) ? "checked" : ""}></label>`).join("")}</div></div>
                      ...
                      <div class="mini-field"><label>Caracteristicas</label><div class="check-list">${["vista mar", "praia", "condominio", "comercial"].map((feature) => /*html*/`<label><input type="checkbox" data-cid="listing" data-message="filter" data-name="feature" value="${feature}" ${filters.features.has(feature) ? "checked" : ""}> ${feature}</label>`).join("")}</div></div>
                    </div>
                  </aside>
                  <div>
                    ...
                    ${compared.length >= 2 ? `<div class="compare-box"><strong>Comparando ${compared.length} imoveis</strong><div class="compare-table">${compared.map((property) => /*html*/`<div><b>${property.title}</b><span>${property.price}</span><span>${property.neighborhood}</span><span>${property.area}m2</span><span>${property.bedrooms} quartos</span><span>${property.bathrooms} banheiros</span><span>${property.parking} vagas</span><span>Condominio ${property.condominium}</span><span>IPTU ${property.iptu}</span></div>`).join("")}</div><button class="ghost-btn whatsapp" type="button" data-route="contato">Enviar comparacao para WhatsApp</button></div>` : `<p class="route-note">Marque dois ou mais imoveis para comparar.</p>`}
