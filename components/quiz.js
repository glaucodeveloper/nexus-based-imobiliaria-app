  const QuizComponent = ({ props }) => {
    ...
    const pickSuggestion = () => {
      ...
      if (answers.profile === "casa") pool = pool.filter((item) => item.kind === "Casa");
      if (answers.profile === "apartamento") pool = pool.filter((item) => item.kind === "Apartamento" || item.kind === "Cobertura");
      if (answers.profile === "terreno") pool = pool.filter((item) => item.kind === "Terreno");
      if (answers.budget === "ate-700") pool = pool.filter((item) => item.priceNumber <= 700000);
      if (answers.budget === "ate-1200") pool = pool.filter((item) => item.priceNumber <= 1200000);
      if (answers.budget === "acima-1200") pool = pool.filter((item) => item.priceNumber >= 1200000);
    ...
    return {
      next(message = {}) {
        ...
        if (message.type === "nextStep" && currentStep && answers[currentStep.id]) {
                ...
                <div class="quiz-layout">
                  <article class="quiz-card">
                    ...
                    <div class="quiz-actions">
                      ${completed
                        ? `<a class="ghost-btn" href="#comprar" data-route="comprar">Ver catalogo completo</a><a class="gold-btn" href="#imovel?propertyId=${encodeURIComponent(suggestion.id)}" data-route="imovel" data-property-id="${suggestion.id}">Abrir sugestao</a>`
